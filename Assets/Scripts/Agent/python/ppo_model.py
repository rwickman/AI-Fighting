...
class PPOModel:
    ...
    def ppo_loss_continuous(self, advantage, old_prediction):
        def loss(y_true, y_pred):
            denom = K.sqrt(2 * np.pi * self.var)
            prob_num = K.exp(- K.square(y_true - y_pred) / (2 * self.var))
            old_prob_num = K.exp(- K.square(y_true - old_prediction) / (2 * self.var))

            prob = prob_num/denom
            old_prob = old_prob_num/denom
            r = prob/(old_prob + 1e-10)

            return -K.mean(K.minimum(r * advantage, K.clip(r, min_value=1 - self.epsilon_clip, max_value=1 + self.epsilon_clip) * advantage))
        return loss
    ...
    def build_actor_and_critic(self):
        ...
        out_actor = tf.keras.layers.Dense(self.num_actions, activation="tanh", kernel_initializer="normal")(x)
        ...
        self.actor.compile(
            optimizer=tf.keras.optimizer.Adam(lr=self.lose_rate),
            loss=[self.ppo_loss_continuous(
                advantage=advantage,
                old_prediction=old_prediction)]
            )
        self.critic.compile(optimzer=tf.keras.optimizer.Adam(lr=self.lose_rate), loss="mse")
